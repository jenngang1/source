...
| search ErrorCategory="Unknow Error"
...
| rex field=err_message "(?<err_class_1>QueryObject creation failed) for: (?<err_category_1>\{\"ClientInfo\":\{\"Context\":\{\".*?):"
| rex field=err_message "(?<err_class_2>QueryObject creation failed) for: (?<err_category_2>\{\"Metadata\":\{\".*?):"
| rex field=err_message "http:(.*?)\/api\/(?<err_class_3>.*?),(.*?)(\")*(Error)*:\s(?<err_category_3>.*?)(\s\()*(;)*"
| rex field=err_message "(?<err_class_4>500 Internal Server Error): \"\{\"errors\":\[\],\"permissionDetails\":\{\"permissions\":\[\{\"errors\":\[(\{\"code\":\")*(?<err_category_4>.*?)\","
| rex field=err_message "(?<err_class_5>(ERROR_CODE 6002 )*We failed to execute the query.)"

| eval err_class = case(NOT isnull(err_class_1), err_class_1, NOT isnull(err_class_2), err_class_2, NOT isnull(err_class_3), err_class_3, NOT isnull(err_class_4), err_class_4, NOT isnull(err_class_5), err_class_5, 1==1, null())

| eval err_category = case(NOT isnull(err_category_1), err_category_1, NOT isnull(err_category_2), err_category_2, NOT isnull(err_category_3), err_category_3, NOT isnull(err_category_4), err_category_4, NOT isnull(err_category_5), err_category_5, 1==1, null())
