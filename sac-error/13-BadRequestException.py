...
| search ErrorCategory="BadRequestException"
...
| eval instance = sac_app_company_id 
| eval correl = sac_app_correlation_id
| eval correl = replace (correl, "-" , "")

| dedup date, time, instance, correl

| rex field=_raw "([0-9]{4}-[0-9]{2}-[0-9]{2}) ([0-9]{2}:[0-9]{2}:[0-9]{2}.[0-9]{3}) ERROR (?<err_no>\d+) \S+ \[(?<url>\S+)\] (?<class_name>\S+)\s*\:"

| rex field=_raw "^(?<skip>(?:[^\]\n]*\]){3})\s*(?P<err_message>.+)" 

| rex field=err_message "(?<err_class_1>Migration failed for query)\s"
| eval err_class_1 = err_class_1."..."
| rex field=err_message "(?<err_class_2>ERROR 4502 Response of external server cannot be parsed)"
| rex field=err_message "(?<token_1>PackageName)\s\((?<err_category_3>\'\$$\$$PackageName\$$\$$\/).*?\)\s(?<token_2>.*)"
| eval err_class_3 = token_1." ".token_2
| rex field=err_message "(?<err_class_5>QueryObject creation failed) for: (?<err_category_5>\{\"Metadata\":\{\".*?):"
| rex field=err_message "(?<err_class_6>QueryObject creation failed) for: (?<err_category_6>\{\"ClientInfo\":\{\"Context\":\{\".*?):"

| eval err_class = case(NOT isnull(err_class_1), err_class_1, NOT isnull(err_class_2), err_class_2, NOT isnull(err_class_3), err_class_3, NOT isnull(err_class_4), err_class_4, NOT isnull(err_class_5), err_class_5, NOT isnull(err_class_6), err_class_6, 1==1, null())

| eval err_category = case(NOT isnull(err_category_1), err_category_1, NOT isnull(err_category_2), err_category_2, NOT isnull(err_category_3), err_category_3, NOT isnull(err_category_4), err_category_4, NOT isnull(err_category_5), err_category_5, NOT isnull(err_category_6), err_category_6, 1==1, null())

| streamstats count as id | eval count = "1"

| rename SFDC AS "dc", err_no AS "Error (number)", url AS "System (url)", class_name AS "class", err_class AS "errorClass", err_category as "errorCategory" 
