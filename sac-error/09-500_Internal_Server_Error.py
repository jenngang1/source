...
index=hcm_sac_app host="*" OR servername="*" 
(sourcetype = app_log_sac)
sac_app_log_level = ERROR 

(
"Error occured while saving labels to DB" OR 
"User authentication validation failed due to invalid authentication details." OR 
"Error retrieving translations from remote API" OR 
"HDI Deployment active: false" OR 
"Request: /qbuilder/sap/fpa/remote/qbuilder/GetServerInfo Unable to parse" OR 
"does not have the correct naming convention!" OR 
"Error occured while extracting story execution details from Ina request." OR 
"Language is not set in the inaRequest" OR 
"Unhandled error in servlet filter processing." OR 
"Token decryption failed." OR 
"Unexpected error occured: com.sap.orca.remote.qbuilder.rest.exceptions.ServerErrorException" OR 
"I/O error on POST request for" OR 
"GenericRemoteAPI call failed. Request URI:" OR 
"403 Forbidden" OR 
"500 Internal Server Error:" OR 
"Query is not activated and will be deleted" OR 
"Package name missing" OR 
"Could not commit JPA transaction" OR 
"com.sap.db.jdbc.exceptions.JDBCDriverException: SAP DBTech JDBC" OR 
"ERROR_CODE5037\"Column does not exist in calculation View\"" OR 
"does not exists in data source" OR 
"Unexpected error occured: org.apache.catalina.connector.ClientAbortException: java.net.SocketTimeoutException" OR 
"Unexpected error occured: java.lang.NullPointerException" OR 
"Cannot forward to error page for request" OR 
"Unexpected error occured: java.util.MissingResourceException" OR 
"Unexpected error occured: org.springframework.beans.factory.BeanCreationException:" OR 
"Rest Client Exception Occurred : org.springframework.web.client.ResourceAccessException" OR 
"SFSF Metadata child node list is empty." OR 
"User authentication service call failed" OR 
"Remote API error: Invalid node Id or error while fetching label" OR 
"406 Not Acceptable" OR 
"java.util.MissingResourceException: Could not load any resource bundle by com.sun.org.apache.xerces.internal.impl.msg.XMLSchemaMessages" OR 
"LogicalModelName is null/empty in logical model" OR 
"c.s.o.r.q.o.s.q.RuntimeMetadataHandler" OR 
"PooledConnection has already been closed." OR 
"File not found:" OR 
"Failed to get the server information:" OR 
"ERROR-5018: Source data source" OR 
"Unexpected error occured: org.apache.catalina.connector.ClientAbortException: java.io.IOException: Broken pipe" OR 
"VCAP_SERVICES is valid JSON, but not a JSON object" OR 
"504 Gateway Time-out" OR 
"Unexpected error occured: com.sap.db.jdbc.exceptions.InternalReconnectException" OR 
"ERROR-5008:" OR 
"Translations: NodeSource information not available for" OR 
"Unable to parse the Accept-Language: en-US,en;q=0.9" OR 
"Right datasource rd5 is used in two joins" OR 
"ERROR-5028:" OR 
"Unexpected error occured: java.lang.ArrayIndexOutOfBoundsException" OR 
"Could not extract response" OR 
"com.sap.db.jdbc.ConnectionSapDB._tryReconnect" OR 
"502 Bad Gateway:" OR 
"401 Bad credentials" OR 
"Ljava.lang.StackTraceElement" OR 
"Unexpected error occured: com.google.gson.JsonSyntaxException: com.google.gson.stream.MalformedJsonException" OR 
"Query not found." OR 
"Error Fetching and Building widget List: A server error has occurred" OR 
"Unable to get data from publicAPI service" OR 
"Exception while processing variables" OR 
"Error during processSynchronization." OR 
"The selection returned more records than the limit in MaxResultRecords" OR 
"Unexpected error occured: org.apache.catalina.connector.ClientAbortException: java.io.IOException: Connection reset by peer" OR 
"QueryObject creation failed for:" OR 
"java.sql.SQLException: Connection has already been closed." OR 
"Unexpected error occured: org.springframework.orm.jpa.JpaSystemException:" OR 
"SAP DBTech JDBC: [258]: insufficient privilege:" OR 
"Error while writing csv export response to DB" OR 
"Unexpected error occured: org.springframework.web.client.ResourceAccessException:" OR 
"Failed to execute an InA query:" OR 
"Empty root node list."
OR "The parallel execution limit per tenant has been exceeded:100"
...
| search ErrorCategory="500 Internal Server Error" 
...
| rex field=err_text "\{\"code\":\"(?<err_class1>.*?)(:)*\",\"message\":\"(?<err_category1>.*?)\","
| rex field=err_text "\{\"code\":\"(?<err_class2>.*?)(:)*\",\"message\":\"(?<err_category2>.*?)[\:\(]"

| rex field=err_text "\<strong\>Error code\:\<\/strong\>(?<err_class3>.*?)(:)*\<br \/\>(.*?)\<strong\>Root cause\:\<\/strong\>(?<err_category3>.*?)(:)*\<br \/\>"

| rex field=err_text "^(?<err_class4>500 Internal Server Error): \[(?<err_category4>no body)\]$"
| rex field=err_text "^(?<err_class5>500 Internal Server Error): \[(.*?)\<title\>(?<err_category5>.*?)\<\/title\>"
| rex field=err_text "^(?<err_class6>500 Internal Server Error): \[(.*?)\<title\>(?<err_category6>.*?)\("

| rex field=err_text "\[\{(?<err_class7>\"cause\":null),(.*?)\[\{\"methodName\":\"(?<err_category7>.*?)\","

| rex field=err_text "\[\{(?<err_class8>\"errors\":\[\]),(.*?),\"nodeId\":\"(?<err_category8>.*?)\","
| rex field=err_text "\[\{(?<err_class9>\"errors\":\[\]),(.*?),\"nodeId\":\"(?<err_category9>.*?)[\#\(]"

| rex field=err_text "\[\{\"(?<err_class10>fault)\":\{\"faultstring\":\"(?<err_category10>.*?);"

| eval err_class=case (NOT isnull(err_class1), err_class1, NOT isnull(err_class2), err_class2, NOT isnull(err_class3), err_class3, NOT isnull(err_class4), err_class4, NOT isnull(err_class5), err_class5, NOT isnull(err_class6), err_class6, NOT isnull(err_class7), err_class7, NOT isnull(err_class8), err_class8, NOT isnull(err_class9), err_class9, NOT isnull(err_class10), err_class10, 1==1, null())
| eval err_class=case(trim(err_class)="500", "500 Internal Server Error", 1==1, err_class)
...
| eval err_category=case (NOT isnull(err_category1), err_category1, NOT isnull(err_category2), err_category2, NOT isnull(err_category3), err_category3, NOT isnull(err_category4), err_category4, NOT isnull(err_category5), err_category5, NOT isnull(err_category6), err_category6, NOT isnull(err_category7), err_category7, NOT isnull(err_category8), err_category8, NOT isnull(err_category9), err_category9, NOT isnull(err_category10), err_category10, 1==1, null())
| eval err_category=substr(err_category, 1, len(mvindex(split(lower(err_category),":"),0)))
