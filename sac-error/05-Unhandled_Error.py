...
| search ErrorCategory="Unhandled Error"
...
| rex field=err_detail "(?<err_class>\S+)\: (?<err_message>\S.*)" 
| eval err_category = case( 
    like(err_message, "%OutOfMemoryError%"), case(len(err_message) > 200, substr(err_message,1,len(mvindex(split(lower(err_message),"("),0))), 1==1, err_message),
    like(err_message, "%Request processing failed%") OR like(err_message, "%Handler dispatch failed;%"), substr(err_message,1,len(mvindex(split(lower(err_message),":"),0))), 
    like(err_message, "%Unsupported or Malformed JWT%"), substr(err_message,1,len(mvindex(split(lower(err_message),"."),0))), 
    like(err_message, "%while trying to invoke the method%"), substr(err_message,1,len(mvindex(split(lower(err_message),")"),0))+1), 
    like(err_message, "%Error creating bean%"), substr(err_message,1,len(mvindex(split(lower(err_message),":"),0))), 
    like(err_message, "%JWT must not be accepted before%"), replace(err_message, "(.*?)"."[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:([0-9]{2}|[-]+)Z".".*", "\1") + "(yyyy-mm-ddThh:mm:ssZ)",
    like(err_message, "%JWT has already expired%"), "JWT has already expired",
    like(err_message, "%Token does not exist%") OR like(err_message, "%Broken pipe%") OR like(err_message, "%Token decryption failed%"), err_message, 
    1==1, err_message) 
