...
| search ErrorCategory="Execution Details Extraction" 
...
| rex field=err_text "^(?<err_class1>Error occured while extracting story execution details from Ina request\..*?)$$"

| eval err_class=case (NOT isnull(err_class1), err_class1, 1==1, null())
...
| rex field=err_detail "^(?<err_category1>java\.lang\.IndexOutOfBoundsException:.*?)$$"

| rex field=err_detail "^(?<err_category2>java\.lang\.NullPointerException:.*?)\("

| eval err_category=case (NOT isnull(err_category1), err_category1, NOT isnull(err_category2), err_category2, 1==1, null())
