...
| search ErrorCategory="PooledConnection has already been closed"
...
| rex field=info_text "^(?<err_class>.*?)\:.*$$"
| rex field=err_text "^.*?\:(?<err_class>.*?)\:.*$$"
| rex field=err_text "^(?<err_class>Error executing SQL).*$$" | eval err_class = err_class + ". . ."
| rex field=err_text "^(?<err_class>Cannot rollback transaction.).*$$"
