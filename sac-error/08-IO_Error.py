...
| search ErrorCategory="I/O Error"

| rex field=_raw "([0-9]{4}-[0-9]{2}-[0-9]{2}) ([0-9]{2}:[0-9]{2}:[0-9]{2}.[0-9]{3}) ERROR (?<err_no>\d+) \S+ \[(?<url>\S+)\] (?<class_name>\S+)\s*\: \[+(?<user_id>\S+)\,(?<instance>\S+)\,(?<other_id>\S+)\]+ \[+(?<GID>\S+)\]+ (?<err_text>.*)" 

| rex field=err_text "(?<err_class1>Rest Client Exception Occurred)\s:"

| rex field=err_text "\"http.*?\/api\/(?<err_class2>.*?)\":\s(?<err_category2>.*?)\s*[\(\;]"

| rex field=err_text "(?<err_class3>QueryObject creation failed)\sfor:"

| eval err_class=case (NOT isnull(err_class1), err_class1, NOT isnull(err_class2), err_class2, NOT isnull(err_class3), err_class3, 1==1, null())
...
| eval err_category=case (NOT isnull(err_category1), err_category1, NOT isnull(err_category2), err_category2, NOT isnull(err_category3), err_category3, 1==1, null())
| eval err_category=if(isnull(err_category2), "Unassigned", trim(err_category))
...
| table id, type, dc, host, servername, server, server_type, instance, date, time, hour, "Error (number)", "System (url)", class, correl, errorClass, errorCategory, count

| stats count by errorClass, errorCategory | sort - count
| chart values(count) as count over errorClass by errorCategory